...
work_dir = 'work_dirs/fair1mv1_bhcl_decoupled_1x'
...
custom_imports = dict(imports=[
        'projects.BHCL.datasets',
        'projects.BHCL.evaluation',
        'projects.BHCL.models',
        'projects.BHCL.utils'
    ],
    allow_failed_imports=False)

val_evaluator = dict(type='LeafNodeDOTAMetric', iou_thrs=[0.5, 0.55, 0.6, 0.65, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95],
                     leaf_indices={{_base_.leaf_indices}})
...
model = dict(
    ...
    rpn_head=dict(
        ...
        ddq_num_classes={{_base_.num_classes}},
        ...
        decouple=decouple),
    roi_head=dict(
        type='DecoupledOrientedAdaMixerDecoder',
        ...
        decouple=decouple,
        hcl_loss=dict(
            type='BalancedHierarchicalContrastiveLoss',
            num_classes={{_base_.num_classes}},
            num_levels=4,
            hierarchical_labels={{_base_.hierarchical_labels}},
            begin_level=1,
            proj_head_input_dim=256,
            proj_head_hidden_dim=512,
            proj_head_output_dim=128,
            temperature=0.1,
            level_weights=None,
            loss_weight=0.6,
            epsilon=0.1,
...
optim_wrapper = dict(
    optimizer=dict(
        _delete_=True, type='AdamW', lr=1e-4, weight_decay=1e-6),
