...
class OrientedFormerDecoderLayer(BBoxHead):
    ...
    def __init__(self,
        ...
        if decouple:
            self_attn_cfg['embed_dims'] *= 2

        # self-attention
        self.tau = nn.Parameter(torch.ones(self_attn_cfg.get('num_heads'), ))
        self.self_attn = MultiheadAttention(**self_attn_cfg)
        if decouple:
            self.self_attn_norm = build_norm_layer(dict(type='LN'), content_dim * 2)[1]
        else:
            self.self_attn_norm = build_norm_layer(dict(type='LN'), content_dim)[1]

        if decouple:
            # cross-attention for classification
            self.o3d_attn_cls = MODELS.build(o3d_attn_cfg)
            self.o3d_attn_norm_cls = build_norm_layer(dict(type='LN'), content_dim)[1]
            # cross-attention for bounding box regression
            self.o3d_attn_reg = MODELS.build(o3d_attn_cfg)
            self.o3d_attn_norm_reg = build_norm_layer(dict(type='LN'), content_dim)[1]

            # FFN for classification
            self.ffn_cls = FFN(**ffn_cfg)
            self.ffn_norm_cls = build_norm_layer(dict(type='LN'), content_dim)[1]
            # FFN for bounding box regression
            self.ffn_reg = FFN(**ffn_cfg)
            self.ffn_norm_reg = build_norm_layer(dict(type='LN'), content_dim)[1]
        else:
            # cross-attention
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            if self.decouple:
                pe = position_embedding(self.decode_box(query_xyzrt), query_content.size(-1) // 8)    # (bs, num_query, 256)
            else:
        ...
        if self.decouple:
            query_content[..., :self.content_dim] = query_content[..., :self.content_dim] + pe    # (num_query, bs, 256)
            query_content[..., self.content_dim:] = query_content[..., self.content_dim:] + pe    # (num_query, bs, 256)
        else:
            query_content = query_content + pe             # (num_query, bs, 256)
        
        query_content = self.self_attn(
            query_content,                            # (num_query, bs, 256)
        ...
        if self.decouple:
            query_content_cls, query_content_reg = query_content[..., :self.content_dim], query_content[..., self.content_dim:]
            # cross-attention for classification
            query_content_cls = self.o3d_attn_cls(
                x, query_content_cls, query_xyzrt, featmap_strides) # (bs, num_query, 256)
            query_content_cls = self.o3d_attn_norm_cls(query_content_cls)
            # cross-attention for bounding box regression
            query_content_reg = self.o3d_attn_reg(
                x, query_content_reg, query_xyzrt, featmap_strides) # (bs, num_query, 256)
            query_content_reg = self.o3d_attn_norm_reg(query_content_reg)
            # FFN for classification
            query_content_cls = self.ffn_norm_cls(self.ffn_cls(query_content_cls)) # (bs, num_query, 256)
            # FFN for bounding box regression
            query_content_reg = self.ffn_norm_reg(self.ffn_reg(query_content_reg)) # (bs, num_query, 256)
        else:
            # cross-attention
        ...
        if self.decouple:
            return cls_score, xyzrt_delta, torch.concat([query_content_cls, query_content_reg], dim=-1).view(N, n_query, -1)
        else:
